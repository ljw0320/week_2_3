...
"""
이진 트리는 무언가를 두 갈래로 나누면서 처리하는 문제에 자주 등장한다.
1. 의사결정 트리
ex) 
회원인가? -> 예/아니오
결제 이력이 있는가? -> 예/아니오
...
2. 우선순위 큐의 힙
- 힙은 완전 이진 트리 구조를 사용
- 예: 응급실 환자 우선수누이, 작업 스케줄링 등

3. 수식 계산
- (A + B) * C 같은 수식을 트리로 표현할 수 있다.
     ...
     / \
    A   B
- 컴파일러가 수식을 해석할 때 이런 트리 구조를 사용

4. 게임 AI/ 선택 구조
- 어떤 행동을 할지 두 가지 선택지로 나누면서 판단하는 구조에 사용 가능
ex)rpg monseter AI
                 플레이어가 보이는가?
                 /              \
               예                아니오
